...
import { 
  getFirestore, doc, setDoc, collection, onSnapshot, addDoc, deleteDoc, getDocs 
...
const appId = typeof (window as any).__app_id !== 'undefined' ? (window as any).__app_id : 'huevos-queens-unified-v3';
const COLORS = ['#0f766e', '#d97706', '#2563eb', '#dc2626', '#7c3aed', '#db2777', '#4b5563'];
const TIPOS_HUEVO = ['Jumbo', 'AAA', 'AA', 'A', 'B', 'C', 'Rotos'];

interface DailyRecord {
  fincaProduccion: Record<string, number>;
  fincaVentas: any[];
  fincaTransfers: Record<string, number>;
  invInicial: Record<string, number>;
  ventas: any[];
  cobros: any[];
  gastos: any[];
  invFinalFisico: Record<string, string | number>;
}

const DIA_VACIO: DailyRecord = {
...
export default function App() {
  ...
  const [alertConfig, setAlertConfig] = useState<{
    ...
    type: string;
    onConfirm: (() => void) | null;
  }>({ visible: false, title: '', message: '', type: 'success', onConfirm: null });
  
  const [toastMessage, setToastMessage] = useState<string>('');

  // Estados de Migración
  const [migrando, setMigrando] = useState<boolean>(false);
  const [statusMigracion, setStatusMigracion] = useState<string>('');

  const [nuevaVentaDist, setNuevaVentaDist] = useState({ 
    cliente: '', cantidad: '', tipo: 'A', precioUnitario: '', pagadoAElla: true, metodoPago: 'Efectivo' 
  });
  const [nuevaVentaFinca, setNuevaVentaFinca] = useState({ 
    cliente: '', cantidad: '', tipo: 'A', precioUnitario: '', metodoPago: 'Efectivo' 
  });
  ...
  const [busquedaDeudor, setBusquedaDeudor] = useState<string>('');
  ...
  useEffect(() => {
    ...
    if (esDiaNuevo) {
      ...
      if (datosAyer && datosAyer.invFinalFisico) {
        const inventarioHeredado: Record<string, number> = {};
        let hayDatos = false;
  ...
  const showAlert = (title: string, message: string, type: string = 'success', onConfirm: (() => void) | null = null) => {
  ...
  const guardarEnFirebase = async (datosActualizados: any, fechaDestino: string = fecha, mostrarMensaje: boolean = true) => {
    try {
      if (!user) {
  ...
  const ejecutarMigracionGeneral = async () => {
    ...
    try {
      ...
      for (const documento of oldDailySnap.docs) {
        ...
        const newDocRef = doc(db, 'artifacts', appId, 'public', 'data', 'daily_records', docId);
        await setDoc(newDocRef, adaptado, { merge: true });
        copiadosDaily++;
      }

      setStatusMigracion('Cargando gastos históricos de granja...');
      const oldExpensesRef = collection(db, 'artifacts', 'huevos-queens-gastos', 'public', 'data', 'expenses');
      ...
      for (const docExp of oldExpensesSnap.docs) {
        ...
        const payload = {
          ...
          fuenteFinanciamiento: dataExp.type === 'ingreso' ? 'Directo Socios' : 'Inyección de Socio',
      ...
      setStatusMigracion(`🎉 ¡MIGRACIÓN COMPLETADA! Se mudaron ${copiadosDaily} días de distribuidora y ${copiadosExpenses} transacciones de granja.`);
  ...
  const handleFincaProduccionChange = (tipo: string, valor: string | number) => {
  ...
  const handleFincaTransferChange = (tipo: string, valor: string | number) => {
  ...
  const registrarIngresoAutomatico = async (descripcion: string, monto: number | string, fDestino: string) => {
    if (!db) return;
    try {
      const payload = {
        type: 'ingreso',
  ...
  const borrarVentaFinca = (id: number, total: number) => {
    showAlert(
      "Confirmar Eliminación", 
      "¿Seguro que deseas borrar esta venta de Finca?", 
      "danger", 
      () => {
        const fincaVentas = (datosDia.fincaVentas || []).filter((v: any) => v.id !== id);
        guardarEnFirebase({ ...datosDia, fincaVentas });
        closeAlert();
  ...
  const handleInvInicialChange = (tipo: string, valor: string | number) => {
    guardarEnFirebase({ ...datosDia, invInicial: { ...datosDia.invInicial, [tipo]: Number(valor) } });
  };

  const handleInvFinalChange = (tipo: string, valor: string | number) => {
  ...
  const borrarVentaDist = (id: number) => {
    showAlert("¿Borrar venta?", "¿Quieres eliminar este registro de venta?", "danger", () => {
      guardarEnFirebase({ ...datosDia, ventas: (datosDia.ventas || []).filter((v: any) => v.id !== id) });
      closeAlert();
      showToast('🗑️ Venta eliminada.');
    });
  };

  const registrarGastoAutomatico = async (descripcion: string, monto: number | string, categoria: string, fuente: string) => {
    if (!db) return;
    try {
      const payload = {
        type: 'gasto_granja',
  ...
  const borrarGastoDist = (id: number) => {
    showAlert("¿Eliminar Gasto?", "¿Quieres quitar este gasto diario?", "danger", () => {
      guardarEnFirebase({ ...datosDia, gastos: (datosDia.gastos || []).filter((g: any) => g.id !== id) });
      closeAlert();
  ...
  const realizarCobroDeuda = async () => {
    ...
    const ventasActualizadas = diaOriginal.ventas.map((v: any) => v.id === deudorSeleccionado.id ? { ...v, abonado: v.abonado + valorAbono } : v);
  ...
  const borrarCobroHoy = (cobro: any) => {
    showAlert("¿Borrar cobro?", "Se revertirá el abono en la deuda del cliente.", "danger", async () => {
      ...
      if (fOriginal) {
        ...
        const ventasRevertidas = diaOriginal.ventas.map((v: any) => v.id === cobro.refVentaId ? { ...v, abonado: Math.max(0, v.abonado - cobro.valor) } : v);
        await guardarEnFirebase({ ...diaOriginal, ventas: ventasRevertidas }, fOriginal, false);
      }

      await guardarEnFirebase({ ...datosDia, cobros: (datosDia.cobros || []).filter((c: any) => c.id !== cobro.id) }, fecha);
      closeAlert();
      showToast('🗑️ Abono revertido.');
    });
  };

  const guardarMovimientoFinanciero = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!db) return;
  ...
  const borrarMovimientoFinanciero = (id: string) => {
    ...
    Object.keys(dbData).forEach(fechaKey => {
      ...
      if (dia.ventas) {
        dia.ventas.forEach((venta: any) => {
          if (Number(venta.abonado || 0) < Number(venta.total || 0)) {
            deudores.push({ 
              ...venta, 
  ...
  const calculosHoy = useMemo(() => {
    ...
    TIPOS_HUEVO.forEach(tipo => {
      ...
      const ventasFinca = (datosDia.fincaVentas || []).filter((v: any) => v.tipo === tipo).reduce((sum: number, v: any) => sum + Number(v.cantidad || 0), 0);
      ...
      const ventasDist = (datosDia.ventas || []).filter((v: any) => v.tipo === tipo).reduce((sum: number, v: any) => sum + Number(v.cantidad || 0), 0);
      
      invTeoricoDist[tipo] = inicial + transfers - ventasDist;
    });

    const ventasEfectivoDist = (datosDia.ventas || []).filter((v: any) => Number(v.abonado || 0) > 0 && v.metodoPago === 'Efectivo').reduce((sum: number, v: any) => sum + Number(v.abonado || 0), 0);
    const ventasNequiDist = (datosDia.ventas || []).filter((v: any) => Number(v.abonado || 0) > 0 && v.metodoPago === 'Nequi').reduce((sum: number, v: any) => sum + Number(v.abonado || 0), 0);
    
    const cobrosEfectivoDist = (datosDia.cobros || []).filter((c: any) => c.metodoPago === 'Efectivo').reduce((sum: number, c: any) => sum + Number(c.valor || 0), 0);
    const cobrosNequiDist = (datosDia.cobros || []).filter((c: any) => c.metodoPago === 'Nequi').reduce((sum: number, c: any) => sum + Number(c.valor || 0), 0);

    const ventasEfectivoFinca = (datosDia.fincaVentas || []).filter((v: any) => v.metodoPago === 'Efectivo').reduce((sum: number, v: any) => sum + Number(v.total || 0), 0);
    const ventasNequiFinca = (datosDia.fincaVentas || []).filter((v: any) => v.metodoPago === 'Nequi').reduce((sum: number, v: any) => sum + Number(v.total || 0), 0);

    const gastosHoy = (datosDia.gastos || []).reduce((sum: number, g: any) => sum + Number(g.valor || 0), 0);
          ...
          {vista === 'dashboard' && (
            <div className="space-y-6 animate-in fade-in duration-300">
              ...
              <div className="bg-teal-50 border border-teal-200 rounded-3xl p-6 shadow-sm">
                <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                  <div className="flex items-center gap-3">
                    <div className="p-3.5 bg-teal-100 rounded-2xl text-teal-800"><Database size={28} /></div>
                    <div>
                      ...
                      <p className="text-xs text-teal-700 font-semibold max-w-xl mt-1 leading-relaxed">
                        Si haces clic abajo, el sistema copiará automáticamente todas las ventas del historial viejo (`registros_diarios_v2`) y los gastos e ingresos de la granja (`expenses`) al nuevo sistema integral unificado sin que pierdas nada.
                      </p>
                    </div>
                  </div>
                  ...
                  >
                    {migrando ? <Loader2 className="animate-spin" size={16} /> : <RefreshCw size={16} />}
                  ...
                  <div className="h-64 w-full">
                    {gastosPorCategoria.length > 0 ? (
                      ...
                      <div className="h-full flex items-center justify-center text-slate-400 text-xs italic">
                        Sin datos de gastos aún (presiona el botón de migración arriba)
                      </div>
                    )}
                  </div>
                </div>
              ...
              <div className="bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-sm">
                <div className="bg-slate-50 p-4 border-b border-slate-100 flex justify-between items-center">
                  ...
                  <span className="text-xs font-bold text-teal-700 bg-teal-50 px-3 py-1 rounded-full font-black">
                    Total Finca Hoy: ${(datosDia.fincaVentas || []).reduce((s: number, v: any) => s + Number(v.total || 0), 0).toLocaleString()}
                  </span>
                </div>
                <div className="overflow-x-auto">
                  <table className="w-full text-sm text-left">
                    ...
                    <tbody>
                      {(datosDia.fincaVentas || []).map((v: any) => (
              ...
              <div className="lg:col-span-4 space-y-6">
                ...
                <div className="bg-blue-50/50 p-5 rounded-3xl border border-blue-200/50 shadow-sm">
                  ...
                  <div className="space-y-2 max-h-60 overflow-y-auto">
                    {listaDeudores.filter((d: any) => d.cliente.toLowerCase().includes(busquedaDeudor.toLowerCase())).map((item: any) => (
                      <div key={item.id} className="bg-white p-3 rounded-2xl border border-blue-100 shadow-sm relative overflow-hidden">
                        ...
                        <div className="flex flex-col gap-1 pl-1">
                          ...
                          {deudorSeleccionado?.id === item.id ? (
                            ...
                            <button 
                              onClick={() => { setDeudorSeleccionado(item); setMontoAbono(String(item.saldoPendiente)); }} 
                ...
                <div className="bg-slate-900 text-white p-5 rounded-3xl border-t-4 border-emerald-500 shadow-xl">
                  ...
                  <div className="space-y-2 mt-4 text-xs font-semibold text-slate-300">
                    <div className="flex justify-between">
                      ...
                      <span>${((datosDia.ventas || []).filter((v: any) => Number(v.abonado || 0) > 0 && v.metodoPago === 'Efectivo').reduce((sum: number, v: any) => sum + Number(v.abonado || 0), 0)).toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>(+) Ventas Hoy (Efectivo Finca):</span>
                      <span>${((datosDia.fincaVentas || []).filter((v: any) => v.metodoPago === 'Efectivo').reduce((sum: number, v: any) => sum + Number(v.total || 0), 0)).toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between text-green-300">
                      <span>(+) Cobro de Cartera (Efectivo):</span>
                      <span>+${((datosDia.cobros || []).filter((c: any) => c.metodoPago === 'Efectivo').reduce((sum: number, c: any) => sum + Number(c.valor || 0), 0)).toLocaleString()}</span>
                    </div>
